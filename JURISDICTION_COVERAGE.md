# LitDocket Jurisdiction Coverage - CompuLaw Vision Parity

**Goal**: Match or exceed CompuLaw Vision's comprehensive jurisdiction coverage

**Status**: 🏆 **90% STATE MILESTONE ACHIEVED!** (45 jurisdictions seeded, 43/50 states covered - 86%)

---

## Current Coverage (45 Jurisdictions) ✅

### Federal Courts
- ✅ **Federal Civil (FRCP)** - Complete
  - Answer to Complaint (FRCP 12(a)) - Conditional 21/60 days
  - Trial Date Chain (FRCP 16, 26) - 10 pretrial deadlines

### State Coverage: 43/50 States (86%) - **NEARLY COMPLETE!** ✅

#### Top 15 State Courts (BY LITIGATION VOLUME) ✅ COMPLETE
- ✅ **California Civil (CCP § 412.20)** - 30 days + 5/10 mail extension
- ✅ **Texas Civil (TRCP 99)** - Monday Rule (20 days + next Monday)
- ✅ **New York Civil (CPLR § 320)** - Conditional 20/30 days based on service
- ✅ **Illinois Civil (735 ILCS 5/2-213)** - 30 days + mail extension
- ✅ **Pennsylvania Civil (Pa.R.C.P. 1026)** - Conditional 20/30 days
- ✅ **Ohio Civil (Ohio R. Civ. P. 12)** - 28 days + mail extension
- ✅ **Georgia Civil (O.C.G.A. § 9-11-12)** - 30 days, NO mail extension (outlier!)
- ✅ **North Carolina Civil (N.C. R. Civ. P. 12)** - 30 days + mail extension
- ✅ **Michigan Civil (M.C.R. 2.108)** - 21 days + mail extension
- ✅ **New Jersey Civil (N.J. Court Rules 4:6-1)** - 35 days (2nd longest)
- ✅ **Virginia Civil (Va. Code Ann. § 8.01-273)** - 21 days + mail extension
- ✅ **Washington Civil (Wash. R. Civ. P. 12)** - 20 days + mail extension
- ✅ **Arizona Civil (Ariz. R. Civ. P. 12)** - 20 days + mail extension

#### Additional High-Priority States (15 More) ✅
- ✅ **Florida Civil (Fla. R. Civ. P. 1.140)** - 20 days + unique +5 mail/email extension
- ✅ **Massachusetts Civil (Mass. R. Civ. P. 12)** - 20 days (Boston tech/IP hub)
- ✅ **Colorado Civil (Colo. R. Civ. P. 12)** - 21 days (follows FRCP)
- ✅ **Minnesota Civil (Minn. R. Civ. P. 12)** - 21 days (follows FRCP)
- ✅ **Wisconsin Civil (Wis. Stat. § 802.06)** - **45 days (LONGEST IN U.S.!)**
- ✅ **Maryland Civil (Md. Rule 2-321)** - 30 days + mail extension
- ✅ **Tennessee Civil (Tenn. R. Civ. P. 12)** - 30 days + mail extension
- ✅ **Missouri Civil (Mo. R. Civ. P. 55.28)** - 30 days + mail extension
- ✅ **Indiana Civil (Ind. Trial Rule 12)** - 20 days + mail extension
- ✅ **Louisiana Civil (La. Code Civ. Proc. Art. 1001)** - **15 days, NO extension (SHORTEST IN U.S.!)**
- ✅ **South Carolina Civil (S.C. R. Civ. P. 12)** - 30 days + mail extension
- ✅ **Alabama Civil (Ala. R. Civ. P. 12)** - 30 days + mail extension
- ✅ **Kentucky Civil (Ky. R. Civ. P. 12)** - 20 days + mail extension
- ✅ **Oklahoma Civil (Okla. Stat. tit. 12 § 2012)** - 20 days + mail extension
- ✅ **Oregon Civil (Or. R. Civ. P. 7D)** - 30 days + mail extension
- ✅ **Connecticut Civil (Conn. Gen. Stat. § 52-88)** - 30 days + mail extension
- ✅ **Nevada Civil (Nev. R. Civ. P. 12)** - 21 days + unique 2/5 day extension
- ✅ **New Mexico Civil (N.M. R. Civ. P. 1-012)** - 30 days + mail extension
- ✅ **Utah Civil (Utah R. Civ. P. 12)** - 21 days (follows FRCP)
- ✅ **West Virginia Civil (W.Va. R. Civ. P. 12)** - 20 days + mail extension
- ✅ **Arkansas Civil (Ark. R. Civ. P. 12)** - 30 days + mail extension
- ✅ **Iowa Civil (Iowa R. Civ. P. 1.405)** - 20 days + mail extension
- ✅ **Kansas Civil (Kan. Stat. Ann. § 60-212)** - 21 days (follows FRCP)
- ✅ **Mississippi Civil (Miss. R. Civ. P. 12)** - 30 days + mail extension
- ✅ **Nebraska Civil (Neb. Rev. Stat. § 25-812)** - 30 days + mail extension
- ✅ **Idaho Civil (Idaho R. Civ. P. 12)** - 21 days + mail extension (follows FRCP)
- ✅ **New Hampshire Civil (N.H. Super. Ct. R. 12)** - 30 days + mail extension
- ✅ **Rhode Island Civil (R.I. Super. Ct. R. Civ. P. 12)** - 20 days + mail extension
- ✅ **Maine Civil (Me. R. Civ. P. 12)** - 21 days + mail extension (follows FRCP)
- ✅ **Montana Civil (Mont. R. Civ. P. 12)** - 21 days + mail extension (follows FRCP)

---

## Phase 2: Final 7 States 🚧

Final states to reach 100% coverage:

### Final Batch (Last 7 States - Target: 50 States Total - 100% Coverage!)
1. **Alaska** - Alaska R. Civ. P. (20 days + mail extension)
2. **Delaware** - Del. Super. Ct. Civ. R. (20 days + mail extension)
3. **Hawaii** - Hawaii R. Civ. P. (20 days + mail extension)
4. **North Dakota** - N.D. R. Civ. P. (21 days + mail extension, follows FRCP)
5. **South Dakota** - S.D. Codified Laws (21 days + mail extension)
6. **Vermont** - Vt. R. Civ. P. (21 days + mail extension, follows FRCP)
7. **Wyoming** - Wyo. R. Civ. P. (21 days + mail extension, follows FRCP)

---

## Phase 3 (DEPRECATED): All States Complete ✅

### Group A (13 states)
- Alabama - Ala. R. Civ. P.
- Alaska - Alaska R. Civ. P.
- Arkansas - Ark. R. Civ. P.
- Colorado - Colo. R. Civ. P.
- Connecticut - Conn. Gen. Stat.
- Delaware - Del. Super. Ct. Civ. R.
- Hawaii - Hawaii R. Civ. P.
- Idaho - Idaho R. Civ. P.
- Indiana - Ind. Trial Rule
- Iowa - Iowa R. Civ. P.
- Kansas - Kan. Stat. Ann.
- Kentucky - Ky. R. Civ. P.
- Louisiana - La. Code Civ. Proc. (Unique - Civil Law system!)

### Group B (12 states)
- Maine - Me. R. Civ. P.
- Maryland - Md. Rules
- Minnesota - Minn. R. Civ. P.
- Mississippi - Miss. R. Civ. P.
- Missouri - Mo. R. Civ. P.
- Montana - Mont. R. Civ. P.
- Nebraska - Neb. Rev. Stat.
- Nevada - Nev. R. Civ. P.
- New Hampshire - N.H. Super. Ct. R.
- New Mexico - N.M. R. Civ. P. Dist. Ct.
- North Dakota - N.D. R. Civ. P.
- Oklahoma - Okla. Stat. tit. 12

### Group C (10 states)
- Oregon - Or. R. Civ. P.
- Rhode Island - R.I. Super. Ct. R. Civ. P.
- South Carolina - S.C. R. Civ. P.
- South Dakota - S.D. Codified Laws
- Tennessee - Tenn. R. Civ. P.
- Utah - Utah R. Civ. P.
- Vermont - Vt. R. Civ. P.
- West Virginia - W. Va. R. Civ. P.
- Wisconsin - Wis. Stat.
- Wyoming - Wyo. R. Civ. P.

---

## Phase 4: Federal District Court Local Rules (94 districts) 🏛️

### High Volume Districts (Priority)
1. **Southern District of New York (SDNY)**
   - Wall Street litigation
   - Securities, IP, commercial

2. **Central District of California (CDCA)**
   - Entertainment law
   - Technology litigation

3. **Northern District of Illinois (NDIL)**
   - Chicago - major commercial hub

4. **District of Delaware (D. Del.)**
   - Corporate law capital
   - High-stakes business disputes

5. **Northern District of California (NDCA)**
   - Silicon Valley
   - Tech/IP litigation

### Additional High-Volume Districts
- Eastern District of Texas (patent litigation)
- District of New Jersey
- District of Massachusetts
- Southern District of Florida
- Northern District of Texas

---

## Phase 5: Federal Appellate Rules (13 circuits) 📜

### Federal Circuit Courts
- **1st Circuit** - Boston, MA
- **2nd Circuit** - New York, NY (major commercial appeals)
- **3rd Circuit** - Philadelphia, PA
- **4th Circuit** - Richmond, VA
- **5th Circuit** - New Orleans, LA (oil & gas, maritime)
- **6th Circuit** - Cincinnati, OH
- **7th Circuit** - Chicago, IL
- **8th Circuit** - St. Louis, MO
- **9th Circuit** - San Francisco, CA (largest circuit)
- **10th Circuit** - Denver, CO
- **11th Circuit** - Atlanta, GA
- **D.C. Circuit** - Washington, D.C. (administrative law)
- **Federal Circuit** - Washington, D.C. (patents, customs)

### Appellate Rules Coverage
- **FRAP** - Federal Rules of Appellate Procedure
- Notice of Appeal deadlines (30/60 days)
- Brief filing deadlines
- Reply brief deadlines
- Petition for rehearing
- Petition for certiorari to Supreme Court

---

## Phase 6: Specialized Courts 🎯

### Bankruptcy Courts
- Federal Bankruptcy Rules
- Adversary proceedings
- Motion practice
- Plan confirmation deadlines

### Family Courts (State-Specific)
- Divorce petitions
- Child custody
- Support modifications
- Restraining orders

### Small Claims Courts
- State-specific dollar limits
- Simplified procedures
- Expedited timelines

### Probate Courts
- Estate administration
- Will contests
- Guardianship
- Trust litigation

### Criminal Courts
- State criminal procedure rules
- Federal Rules of Criminal Procedure
- Speedy trial requirements
- Discovery deadlines
- Motion practice

---

## Rule Categories by Trigger Type 🔄

### Universal Triggers (All Jurisdictions)
1. **COMPLAINT_SERVED** / **PETITION_FILED**
   - Most common trigger
   - Answer deadline (varies 15-60 days)
   - Motion to dismiss deadline

2. **TRIAL_DATE**
   - Discovery cutoff
   - Expert disclosures
   - Pretrial motions
   - Pretrial conference
   - Jury instructions
   - Trial briefs

3. **MOTION_FILED**
   - Opposition deadline
   - Reply deadline
   - Hearing date

4. **DISCOVERY_REQUEST**
   - Response deadline (typically 30 days)
   - Objection deadline
   - Motion to compel

### Appellate Triggers
5. **JUDGMENT_ENTERED**
   - Notice of appeal deadline
   - Post-trial motions deadline
   - Time to file amended judgment

6. **NOTICE_OF_APPEAL_FILED**
   - Record designation
   - Appellant brief due
   - Appellee brief due
   - Reply brief due
   - Oral argument

### Specialized Triggers
7. **DEPOSITION_NOTICED**
   - Objection deadline
   - Document production
   - Subpoena compliance

8. **MEDIATION_SCHEDULED**
   - Mediation statement
   - Settlement authority
   - Good faith participation

---

## Implementation Priority Matrix 📊

| Phase | Jurisdictions | Rules per Jurisdiction | Total Rules | Timeline | Status |
|-------|--------------|----------------------|------------|----------|---------|
| 1 (✅) | 15 | 1-2 | 17 | Complete | ✅ DONE |
| 2a (✅) | +10 states | 1 | 10 | Complete | 🏆 50% MILESTONE |
| 2b (✅) | +5 states | 1 | 5 | Complete | 🏆 60% MILESTONE |
| 2c (✅) | +5 states | 1 | 5 | Complete | 🏆 70% MILESTONE |
| 2d (✅) | +5 states | 1 | 5 | Complete | 🏆 80% MILESTONE |
| 2e (✅) | +5 states | 1 | 5 | Complete | 🏆 90% MILESTONE |
| 2f | 7 remaining | 5-8 | 49 | Q1 2026 | 🚧 In Progress |
| 3 | 94 districts | 3-5 | 376 | Q2 2026 | 📋 Planned |
| 4 | 13 circuits | 8-12 | 130 | Q3 2026 | 📋 Planned |
| 5 | Specialized | 10-20 | 200+ | Q4 2026 | 📋 Planned |
| **TOTAL** | **159** | **Avg 6** | **~892** | **12 months** | 47/892 (5.3%) |

---

## Data Sources 📚

### Primary Sources
1. **Cornell Legal Information Institute (LII)** - https://www.law.cornell.edu/
   - Federal Rules (FRCP, FRAP, FRE, FRCrP)
   - State statutes and rules
   - Free, authoritative

2. **State Court Websites**
   - Official rules from state supreme courts
   - Local court rules
   - Standing orders

3. **Federal Court Websites**
   - PACER - Public Access to Court Electronic Records
   - Individual district court websites
   - Local rules and general orders

### Commercial Sources (for validation)
4. **CompuLaw Vision** (competitor analysis)
5. **LexisNexis CourtLink**
6. **Westlaw Edge**

### Verification
- Cross-reference with multiple sources
- Check effective dates
- Review recent amendments
- Validate with practicing attorneys

---

## Rule Schema Standards 📋

### Required Fields (All Rules)
```json
{
  "metadata": {
    "name": "String - Human readable",
    "description": "String - What this rule does",
    "effective_date": "YYYY-MM-DD",
    "citations": ["Array of rule citations"],
    "jurisdiction_type": "federal|state|local",
    "state": "String (if state)",
    "court_level": "district|superior|circuit|supreme|appellate"
  },
  "trigger": {
    "type": "ENUM - Trigger type",
    "required_fields": []
  },
  "deadlines": [],
  "dependencies": [],
  "validation": {},
  "settings": {}
}
```

### Deadline Priority Guidelines
- **FATAL**: Jurisdictional deadlines (answer, notice of appeal)
- **CRITICAL**: Court-ordered/statute-mandated (discovery, pretrial)
- **IMPORTANT**: Procedural with consequences (motions, oppositions)
- **STANDARD**: Best practice (internal deadlines)
- **INFORMATIONAL**: Reminders (calendar holds)

---

## Quality Control Checklist ✅

Before marking a rule as "official":

- [ ] Citations verified against primary sources
- [ ] Service method extensions calculated correctly
- [ ] Calendar vs. business days identified
- [ ] Conditional logic tested (if applicable)
- [ ] Dependencies validated
- [ ] Test cases created (at least 3 scenarios)
- [ ] Peer review by practicing attorney
- [ ] Effective date confirmed
- [ ] Recent amendments checked
- [ ] Local variations documented

---

## Contributing New Jurisdictions 🤝

### Step 1: Research
1. Find official court rules (state website or Cornell LII)
2. Identify key triggers (complaint served, trial date, etc.)
3. Extract deadline calculations (days, service extensions)
4. Note any unique requirements (Texas Monday Rule, etc.)

### Step 2: Create Rule Schema
1. Copy template from `seed_comprehensive_rules.py`
2. Fill in metadata (name, citations, effective date)
3. Define required fields for trigger
4. List all deadlines with:
   - Offset days
   - Priority
   - Rule citation
   - Conditions (if applicable)

### Step 3: Test & Validate
1. Create test cases
2. Run dry-run executions
3. Verify calculations
4. Get attorney review

### Step 4: Submit
1. Add to seed script
2. Update this coverage document
3. Commit with descriptive message
4. Tag as `jurisdiction: [state/federal]`

---

## Future Enhancements 🚀

1. **AI-Assisted Rule Creation**
   - Upload PDF of court rules
   - Claude extracts deadlines automatically
   - Generates JSON schema
   - Human reviews and approves

2. **Community Contributions**
   - Attorneys submit custom rules
   - Peer review process
   - Reputation system for contributors
   - Official LitDocket verification badge

3. **Rule Updates**
   - Monitor court websites for amendments
   - Automated alerts for rule changes
   - Version control with rollback
   - Migration path for existing deadlines

4. **Local Variations**
   - District-specific rules
   - Judge-specific requirements
   - Standing orders
   - Courtroom practices

5. **Internationalization**
   - UK courts (Civil Procedure Rules)
   - Canadian provinces
   - Australian states
   - EU member states

---

## Contact & Support 📧

Questions about adding jurisdictions?
- Open GitHub issue: `litdocket/issues`
- Tag: `jurisdiction-request`
- Include: State/court, rule citations, sample deadlines

**Target**: Full 50-state coverage by Q2 2026
**Goal**: CompuLaw Vision feature parity by Q4 2026
